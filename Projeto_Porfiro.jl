...
html"""
<p align="center">
	<img src="https://github.com/fnaghetini/3D-Resource-Estimation/raw/main/img/Figura_01.png" alt="Figura_01">
</p>
...
begin
	# Importação dos dados de furo como um dataframe
...
html"""
<p align="center">
	<img src="https://github.com/fnaghetini/3D-Resource-Estimation/raw/main/img/Figura_02.png" alt="Figura_02">
</p>
...
begin
	# Renomendo colunas do dataframe
	...
	# Visualização das 5 primeiras linhas do dataframe renomeado
	first(dh, 5)
end
...
begin
	# Deletando valores faltantes de Cu do banco de dados
	dropmissing!(dh, disallowmissing=true)
	
	# Sumário estatístico do banco de dados após a exlusão de valores faltantes
	describe(dh)
end
...
begin
	# Cálculo de estatísticas da variável Cu
	X̅ = mean(dh.CU) # média
	S² = var(dh.CU) # variância
	S = std(dh.CU) # desvio padrão
	Cᵥ = variation(dh.CU) # coeficiente de variação
	IQR = iqr(dh.CU) # distância interquartil (IQR)
	q = quantile(dh.CU, [0.1, 0.25, 0.50, 0.75, 0.90]) # quantis
	Skew = skewness(dh.CU) # coeficiente de assimetria
	Kurt = kurtosis(dh.CU) # curtose
	
	# Criação do sumário estatístico completo para o Cu (dataframe)
...
begin
	...
	# Cálculo dos quantis da variável A0 (azimute)
	qₐ = quantile(dh.A0, [0.25, 0.75])

	# Criação do sumário estatístico para a variável A0
...
begin
	...
	# Cálculo dos quantis da variável B0 (dip)
	qₚ = quantile(dh.B0, [0.25, 0.75])

	# Criação do sumário estatístico para a variável B0
...
# Criação do sumário estatístico do Cu por litologia
dh |> @groupby(_.LITH) |>
...
# Histograma da variável Cu por litologia
...
begin
	...
	# Linha horizontal tracejada cinza (P90 TnP)
	hline!([P90_TnP], color="gray", ls=:dash, label=false)
	# Linha vertical tracejada cinza (P90 MzP)
...
begin
	...
	# Criando um dataframe a partir dos furos apenas com as coordenadas e o Cu
	dh_temp = dh[:,[:X,:Y,:Z,:CU]]
	
	# Georreferenciando o novo dataframe criado acima
	dh_georef = georef(dh_temp, (:X,:Y,:Z))
end

# ╔═╡ f14a3690-698d-11eb-0a3f-3f28c067f035
# Coordenadas dos furos georreferenciados
domain(dh_georef)

# ╔═╡ f243ffe0-698d-11eb-1c71-05f50abc7fd4
# Valores de Cu (única variável) dos furos georreferenciados
values(dh_georef)
...
html"""
<p align="center">
	<img src="https://github.com/fnaghetini/3D-Resource-Estimation/raw/main/img/Figura_03.png" alt="Figura_03">
</p>
...
html"""
<p align="center">
	<img src="https://github.com/fnaghetini/3D-Resource-Estimation/raw/main/img/Figura_04.png" alt="Figura_04">
</p>
...
# Sumário estatístico original do Cu apenas com algumas estatísticas
...
begin
	...
	# Cálculo dos quantis declusterizados de Cu
	q_dec = quantile(dh_georef, :CU, [0.1, 0.25, 0.5, 0.75, 0.9])
	
	# Criação do sumário estatístico declusterizado de Cu (dataframe)
	sum_dec = DataFrame(
							Variável = :CU_dec,
...
begin
	...
	# Linha vertical tracejada verde (mediana original)
	vline!([sum_cu[:,:P50]], label="P50 Original",
		   color="green", ls=:dashdot, linewidth=1.5)
	
	# Linha vertical contínua verde (mediana declusterizada)
...
begin
	...
	# Criação de partições nos furos de dimensões Γ₁,Γ₂,Γ₃
	Ω = partition(dh_georef, BlockPartition(Γ₁,Γ₂,Γ₃))
	
	# Lista vazia de médias por janela
	medias_locais = []
	# Lista vazia de desvios padrões por janela
	DP_locais = []
	# Número de elementos dentro da lista de médias e DPs
	num_elem = []
	
	# Alimentação das listas vazias de acordo com as dimensões de partição
	for i in 1:length(Ω)
...
begin
	# Georreferenciamento das amostras de GnP
	GnP_georef = georef(GnP[:, [:X,:Y,:Z,:CU]], (:X,:Y,:Z))
	# Georreferenciamento das amostras de MzP
	MzP_georef = georef(MzP[:,[:X,:Y,:Z,:CU]], (:X,:Y,:Z))
	# Georreferenciamento das amostras de TnP
...
html"""
<p align="center">
	<img src="https://github.com/fnaghetini/3D-Resource-Estimation/raw/main/img/Figura_05.png" alt="Figura_05">
</p>
...
html"""
<p align="center">
	<img src="https://github.com/fnaghetini/3D-Resource-Estimation/raw/main/img/Figura_06.png" alt="Figura_06">
</p>
...
<p align="center">
	...
	<img src="https://github.com/fnaghetini/3D-Resource-Estimation/raw/main/img/Figura_07.png">
</p>
...
> **dh_comp = composite(dh, interval=10.0, mincomp=5.0, mode=:equalcomp)**
...
html"""
<p align="center">
	<img src="https://github.com/fnaghetini/3D-Resource-Estimation/raw/main/img/Figura_08.png" alt="Figura_08">
</p>
...
> **dh_comp = composite(dh, interval=10.0, mincomp=5.0, mode=:nodiscard)**
"""

# ╔═╡ 27d99bb0-6e25-11eb-2ada-efd7d4d70e75
html"""
<p align="center">
	<img src="https://github.com/fnaghetini/3D-Resource-Estimation/raw/main/img/Figura_09.png" alt="Figura_09">
</p>
...
html"""
<p align="center">
	<img src="https://github.com/fnaghetini/3D-Resource-Estimation/raw/main/img/Figura_10.png" alt="Figura_10">
</p>
...
begin
	...
	# Cálculo do limiar superior em que P75 + 1.5(IQR)
	cen_1 = (sum_cu.P75 + 1.5*(sum_cu.IQR))[1]
	
	# Cálculo do limiar superior em que X̅ + 6.S
	cen_2 = (sum_cu.X̅ + (6 * sum_cu.S))[1]
	
	# Cálculo do limiar superior em que X̅ + 2.S
	cen_3 = (sum_cu.X̅ + (2 * sum_cu.S))[1]
	
	# Cálculo do limiar superior em que 4.X̅
	cen_4 = (4 * sum_cu.X̅)[1]
	
	# Cálculo do limiar superior em que P95
...
md"""
Limiar Superior: $(@bind cen_6 Slider(0.20:0.01:3.43, default=2.70, show_value=true)) %
"""

# ╔═╡ e3eadb70-6e2f-11eb-1c38-0bfc615d6559
begin
	
	# Plotagem da curva de frequência acumulada do Cu
	plot(ecdf(dh.CU), color=:red, label=false,
	 xlabel="Cu (%)", ylabel="Frequência Acumulada Relativa")
	
	# Linha vertical tracejada cinza (limiar superior para o cenário 6)
...
begin
	...
	# Substituição dos valores de Cu maiores que cen_1 por cen_1
	win_cen_1 = replace(v -> v > cen_1 ? cen_1 : v, dh.CU)
	
	# Substituição dos valores de Cu maiores que cen_2 por cen_2
	win_cen_2 = replace(v -> v > cen_2 ? cen_2 : v, dh.CU)
	
	# Substituição dos valores de Cu maiores que cen_3 por cen_3
	win_cen_3 = replace(v -> v > cen_3 ? cen_3 : v, dh.CU)
	
	# Substituição dos valores de Cu maiores que cen_4 por cen_4
	win_cen_4 = replace(v -> v > cen_4 ? cen_4 : v, dh.CU)
	
	# Substituição dos valores de Cu maiores que cen_5 por cen_5
	win_cen_5 = replace(v -> v > cen_5 ? cen_5 : v, dh.CU)
	
	# Substituição dos valores de Cu maiores que cen_6 por cen_6
...
begin
	...
	# Média não capeada do Cu
	media_orig = sum_cu.X̅[1]
	
	# Média capeada de Cu para o cenário 1
	media_cen_1 = round(mean(win_cen_1), digits=2)
	# Média capeada de Cu para o cenário 2
	media_cen_2 = round(mean(win_cen_2), digits=2)
	# Média capeada de Cu para o cenário 3
	media_cen_3 = round(mean(win_cen_3), digits=2)
	# Média capeada de Cu para o cenário 4
	media_cen_4 = round(mean(win_cen_4), digits=2)
	# Média capeada de Cu para o cenário 5
	media_cen_5 = round(mean(win_cen_5), digits=2)
	# Média capeada de Cu para o cenário 6
	media_cen_6 = round(mean(win_cen_6), digits=2)
	
	# Cálculo da coluna de diferença % entre média original e média capeada
...
html"""
<p align="center">
	<img src="https://github.com/fnaghetini/3D-Resource-Estimation/raw/main/img/Figura_11.png" alt="Figura_11">
</p>
...
html"""
<p align="center">
	<img src="https://github.com/fnaghetini/3D-Resource-Estimation/raw/main/img/Figura_12.png" alt="Figura_12">
</p>
...
html"""
<p align="center">
	<img src="https://github.com/fnaghetini/3D-Resource-Estimation/raw/main/img/Figura_13.png" alt="Figura_13">
</p>
...
begin
	...
	# Criação do modelo teórico esférico
	γ_sph = SphericalVariogram(nugget=Float64(sph_c₀),
								range=Float64(sph_a),
								sill=Float64(sph_s))
	
	# Plotagem do modelo esférico
...
begin
	...
	# Criação do modelo teórico exponencial
	γ_exp = ExponentialVariogram(nugget=Float64(exp_c₀),
								range=Float64(exp_a),
								sill=Float64(exp_s))
	
	# Plotagem do modelo exponencial
...
html"""
<p align="center">
	<img src="https://github.com/fnaghetini/3D-Resource-Estimation/raw/main/img/Figura_14.png" alt="Figura_14">
</p>
...
function polar2cart(azi, dip)
	azi_rad = deg2rad(azi) # conversão do azimute de entrada para radianos
	dip_rad = deg2rad(dip) # conversão do dip de entrada para radianos
	x = sin(azi_rad) * cos(dip_rad) # cálculo da coordenada cartesiana x
	y = cos(azi_rad) * cos(dip_rad) # cálculo da coordenada cartesiana y
	z = (sin(dip_rad)) * -1 # cálculo da coordenada cartesiana z
	return (x, y, z) # retorna uma tupla com as coordenadas cartesianas (x, y, z)
end
...
begin
	...
	# Cálculo variograma direcional 150/55 (down hole) do Cu
...
html"""
<p align="center">
	<img src="https://github.com/fnaghetini/3D-Resource-Estimation/raw/main/img/Figura_15.png" alt="Figura_15">
</p>

<p align="center">
	<b>Figura 15</b>: Regra da mão esquerda, utilizada como padrão de rotações de variogramas e volumes de busca no software Studio RM.
</p>
"""
...
html"""
<p align="center">
	<img src="https://github.com/fnaghetini/3D-Resource-Estimation/raw/main/img/Figura_16.png" alt="Figura_16">
</p>
...
html"""
<p align="center">
	<img src="https://github.com/fnaghetini/3D-Resource-Estimation/raw/main/img/Figura_17.png" alt="Figura_17">
</p>
